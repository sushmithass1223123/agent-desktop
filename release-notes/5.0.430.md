...
-   Workcode widget Data config changed (Source changed to Role: 'interaction' | 'supervisor') - `config`
...
-   `Columns` To confgure the columns to be displayed for particular section.
    ...
    Available `Columns` for each table are:
...
#### Added `Duration` property in `twc-home` and `twc-supervisor` content widget

-   This will take the dashboard data from server for the configured period of time.
