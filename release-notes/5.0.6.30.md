...
> **NOTE**: There is no change in the config key except the casing is changed to camel casing. The app will still support the existing casing for a few more releases to have backward compatibility. If the existing case is used, please make sure to change the spelling issue fix ([see more](#ad-theming))
